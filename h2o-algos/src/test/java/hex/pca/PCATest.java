...
categorical columns.  As soon as we introduced categorical columns,  disagreement between original PCA
and wide data set PCA increases.  However, both methods achieve same accuracy when we project the projected data back
to the original dataset.  Hence, it is okay in this case for them not to agree closely.
 */
  @Test public void testWideDataSetsWithNAs() throws InterruptedException, ExecutionException {
    ...
    try {
      ...
      TestUtil.checkStddev(modelW._output._std_deviation, modelN._output._std_deviation, 1.5);
      boolean[] flippedEig = TestUtil.checkEigvec( modelW._output._eigenvectors, modelN._output._eigenvectors, 2);
  ...
  @Test public void testWideDataSets() throws InterruptedException, ExecutionException {
    ...
    try {
      ...
      TestUtil.checkStddev(modelW._output._std_deviation, modelN._output._std_deviation, 1.5);
      boolean[] flippedEig = TestUtil.checkEigvec( modelW._output._eigenvectors, modelN._output._eigenvectors, 2);
      TestUtil.checkProjection(scoreN, scoreW, 10, flippedEig);
      // Build a POJO, check results with original PCA
      Assert.assertTrue(modelN.testJavaScoring(train,scoreN,1e-5));
      // Build a POJO, check results with wide dataset PCA
      Assert.assertTrue(modelW.testJavaScoring(train,scoreW,1e-5));
  ...
  compare the eigenvalues/vectors from both methods and they should agree.  Dataset contains numerical and
  categorical columns.  As soon as we introduced categorical columns,  disagreement between original PCA
  and wide data set PCA increases.
   */
  @Test public void testWideDataSetsSmallData() throws InterruptedException, ExecutionException {
    ...
    try {
      ...
      // compare eigenvectors and eigenvalues generated by original PCA and wide dataset PCA.
      TestUtil.checkStddev(modelW._output._std_deviation, modelN._output._std_deviation, 0.05);
      boolean[] flippedEig = TestUtil.checkEigvec( modelW._output._eigenvectors, modelN._output._eigenvectors, 1e-3);
      TestUtil.checkProjection(scoreW, scoreN, TOLERANCE, flippedEig);
      // Build a POJO, check results with original PCA
      Assert.assertTrue(modelN.testJavaScoring(train,scoreN,1e-5));
      // Build a POJO, check results with wide dataset PCA
